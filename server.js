...
app.post(
    ...
    async (req, res) => {
        ...
        if (!validationErrors.isEmpty()) {
            return res.status(400).json({
                ...
                errors: validationErrors
                    .array()
                    .map(({ msg: message, param }) => ({ param, message }))
            });
        }
        ...
        try {
            ...
            const message =
                typeof err.body === "object" && "message" in err.body
                    ? err.body.message
                    : err.message;
...
app.put(
    ...
    [
        oneOf(
            [
                checkBody("medical_history")
                    .not()
                    .isEmpty(),
                checkBody("office_visit_notes")
                    .not()
                    .isEmpty(),
                checkBody("perscription")
                    .not()
                    .isEmpty(),
                checkBody("insurer_reply")
                    .not()
                    .isEmpty()
            ],
